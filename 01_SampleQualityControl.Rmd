...
[@Luijk2014] package that we developed requires the targets data frame to store IDAT file root names in a `Basename` column. Sometimes, data comes with a sample sheet to faciliate this, but in this case the information needs to be extracted from the supplementary file column instead. We also extract Sentrix Position from this information and store it in the `targets` object for later.
...
targets$Sentrix_Position <- substring(targets$Basename,23,28)
head(targets[c(38,42,44)])
```
...
Our data is now an `RGset` object that can used for visualization.
