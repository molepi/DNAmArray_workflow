...
The [**MethylAid**](http://bioconductor.org/packages/MethylAid)<sup>1</sup> package, which will be used for quality control in this workflow, requires the targets data frame to contain a `Basename` variable that points to IDAT file names. Sometimes, data comes with a sample sheet to facilitate this, but in this example the information is extracted from the supplementary file column instead. 
...
```{r}
load("./data/sData_example_small.Rdata")
```
...
Plots to visualize sample quality are included in the `DNAmArray` package and their use is outlined below.

First, a list of QC probes needs to be initialized.
...
<sup>1</sup> van Iterson, M., Tobi, E.W., Slieker, R.C., <i>et al</i>. [MethylAid: visual and interactive quality control of large Illumina 450k datasets](https://doi.org/10.1093/bioinformatics/btu566). <i>Bioinformatics</i> <b>30</b>, 23 (2014).
