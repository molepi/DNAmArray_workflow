...
On this page, we demonstrate some different methods for inspecting methylation array data with an emphasis on fast, efficient tools for large data sets.
...
colorLeafs <- function(x) {
  if (is.leaf(x) && attr(x, "label") %in% interestObs) {
    attr(x, "nodePar") <- list(lab.cex=0.8, lab.col="red")
  }
  return(x)
} 
