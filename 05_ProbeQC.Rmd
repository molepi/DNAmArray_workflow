...
Since functional noramlization doesn't accept NA values, the normalized GenomicRatioSet and filtered RGChannelSet need to be merged. For this reason, we provide a `reduce` function in DNAmArray.
...
```{r 513betas}
betas <- betas[anno$cpg, colnames(betas) %in% rownames(targets)]
```
...
```{r 514targets}
targets <- targets[rownames(targets) %in% colnames(betas), ]
